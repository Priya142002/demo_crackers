  ...
  try {
    ...
    if (response.data && response.data.statusCode === 200) {
      ...
      if (docResponse.data.statusCode === 200) {
        ...
        // Send WhatsApp if URL exists
        if (documentUrl) {
          try {
            const whatsappPayload = {
              to: formData.whatsappNumber || formData.contactNumber,
              pdfUrl: documentUrl,
              text: `Sri Gokilaa Crackers ${orderId}`,
              orderNumber: orderId,
      ...
      setIsProcessing(false); // Stop processing after success
...
const generateOrderPDF = (orderId) => {
  ...
  const addPageHeader = () => {
    ...
    // Company name - Sri Goillaa Crackers
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("Sri Gokilaa Crackers", pageWidth/2, 16, { align: "center" });
  ...
  // Customer Details Section with bold labels
...
// Email - only show if exists, with added spacing
if (formData.email) {
  doc.setFont("helvetica", "bold");
  ...
  doc.text("srigokilaacrackers0@gmail.com", pageWidth/2, yPos + 8, { align: "center" });
  doc.text(`Generated: ${new Date().toLocaleString('en-GB')}`, pageWidth - margin, yPos + 8, { align: "right" });

  // Save for user
  doc.save(`${orderId}_SriGokilaaCrackers.pdf`);
  ...
  try {
    ...
    const response = await axios.get(`${API_BASE_URL}api/Crackers/GetProductSummary?CompanyId=1`);
   ...
   const handleQuantityChange = (productId, value) => {
    ...
    setProductCategories((prev) => {
      ...
      newCategories.forEach((category) =>
        category.products.forEach((p) => {
          if (p.id === productId) p.quantity = newQuantity;
        })
      );
      return newCategories;
    });
...
<motion.div 
    ...
    style={{
        position: 'relative',
        height: '160px', // Keep the height as is for now
        padding: '0.5rem', // Reduced from 0 to 0.5rem
        margin: '0',
...
>
   ...
   style={{
    ...
    gap: mobileMediaQuery.matches ? '0.6rem' : '0.8rem', // Reduced gap
    width: 'calc(100% - 1.6rem)', // Adjusted width calculation
    marginLeft: '0.1rem',
    marginRight: '1rem',
    position: 'sticky',
    top: '80px',
    zIndex: 100,
    marginTop: '10px', // Removed extra margin-top
}}
>
    {/* Search Input */}
    <div style={{
...
{/* Products Table */}
<motion.div 
 className="product-table-anchor"  // 👈 Add this class
    initial={{ opacity: 0, y: 20 }}
    animate={{ opacity: 1, y: 0 }}
        ...
        <table style={{
            ...
            <thead>
                <tr style={{ 
                       ...
                       zIndex: 2, // Lower than popups but above content
                    ...
                    filteredCategories.map((category, catIndex) => (
                        <React.Fragment key={`cat-${catIndex}`}>
                            ...
                            <tr style={{
                                ...
                                top: '32px', // Below the main table header
                                zIndex: 1, // Below main header but above content
                            }}>
                                <td style={{
...
<div style={{ ...styles.card, flex: isMobile ? 1 : 3 }}>
  ...
  style={{
    position: 'absolute',
    right: '1px',  // Moved slightly right from edge
    top: '-4px',    // Moved slightly down from top
    cursor: 'pointer',
    color: '#b91c1c', // Darker red color
    zIndex: 1,
    background: 'none',
    border: 'none',
    padding: '2px',
    ...
    width: '18px',
    height: '18px',
    fontWeight: 'bold' // Made icon bold
  }}
  onClick={() => {
    setCartProducts(prevProducts =>
  ...
  <FiX />
          ...
          {!isMobile && (
            <div style={styles.productPrice}>
              ...
              <div>₹{formatPrice(product.subtotal)}</div>
            </div>
          )}
        </li>
      ))}
...
<button 
  ...
  style={{
    ...
    backgroundColor: isProcessing ? '#4CAF50' : 'green',

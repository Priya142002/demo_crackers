...
import pro_bg from "../assets/product.jpg";
...
// Remove this useEffect that was using location.state
// useEffect(() => {
...
const handleConfirmOrder = async () => {
  ...
  const payload = {
    CompanyId: 1, // Add this line to include CompanyId
 ...
 const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data && response.data.statusCode === 200) {
        ...
        const pdfBlob = generateOrderPDF(); // This saves the PDF and returns blob
            ...
            if (!documentUrl) {
              ...
              try {
                const whatsappPayload = {
                  ...
                  pdfUrl: documentUrl,
                  text: `Sri Gokilaa Crackers Order ${orderId}`,
                  orderNumber: orderId,
                };
...
const generateOrderPDF = async (orderId) => {
  ...
  // Section header - made bold as requested
 ...
 doc.save(`${orderId}_Sri Gokilaa Crackers_Order.pdf`);
  ...
  try {
    ...
    formDataToSend.append("OrderId", orderId);
    formDataToSend.append("Document", pdfBlob, `Order_${orderId}.pdf`);
    formDataToSend.append("CompanyId", 1); // 🔴 CRITICAL: Add CompanyId here
